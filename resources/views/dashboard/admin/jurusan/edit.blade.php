...
<head>
    ...
    <title>Edit Jurusan - TOEICLY Admin</title>
    <!-- Tailwind CSS -->
    ...
    <style>
        /* Gunakan style yang sama seperti form tambah/edit kampus */
        body {
...
<body>
   <aside class="sidebar flex flex-col">
    ...
    <main>
        <h1 class="text-4xl font-bold mb-6 text-purple-700">Edit Jurusan</h1>
        ...
        <form action="{{ route('jurusan.update', $jurusan->jurusan_id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="jurusan_kode">Kode Jurusan</label>
                <input type="text" id="jurusan_kode" name="jurusan_kode" maxlength="10" required
                    value="{{ old('jurusan_kode', $jurusan->jurusan_kode) }}">
            </div>

            <div>
                <label for="jurusan_nama">Nama Jurusan</label>
                <input type="text" id="jurusan_nama" name="jurusan_nama" maxlength="100" required
                    value="{{ old('jurusan_nama', $jurusan->jurusan_nama) }}">
            </div>
            ...
            <div>
                ...
                <select id="kampus_id" name="kampus_id" required>
                    ...
                    @foreach($kampus as $k)
                        <option value="{{ $k->kampus_id }}" {{ old('kampus_id', $jurusan->kampus_id) == $k->kampus_id ? 'selected' : '' }}>
                            {{ $k->kampus_nama }}
                        </option>
                    @endforeach
                </select>
